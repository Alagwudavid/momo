...
const Search = () => {
  ...
  useEffect(() => {
    if (query) {
      const combinedList = [...categories, ...products];
      const filteredSuggestions = combinedList.filter((item) =>
  ...
  return (
    <div className="relative flex flex-col items-center lg:w-[500px] md:w-96 min-w-64">
      ...
      {isSuggestionsVisible && (
        <ul
          className="absolute z-10 top-full w-full bg-white border border-gray-300 rounded-lg mt-2 max-h-60 overflow-y-auto"
          aria-live="polite"
        >
