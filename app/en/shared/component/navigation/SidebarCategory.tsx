...
const SidebarCategory = () => {
  ...
  return (
    <div className="flex flex-col items-center font-medium" onMouseLeave={handleMouseLeave}>
      <div className="relative border-[1px] rounded-lg mt-5 bg-[#eae8ff]">
          ...
          {categories.map((category) => (
            <>
              <li
                key={category.id}
                onMouseEnter={() =>
                  handleCategoryMouseEnter(category.id.toString())
                }
                className="relative cursor-pointer px-4 py-2"
              >
                <span className="hover:text-[#390278]">{category.name}</span>
                            ...
                            {coItemsForSubcategory.length > 0 && (
                              <ul className="">
                                {coItemsForSubcategory.map((item) => (
                                  <li
                                    key={item.id}
                                    className="px-4 py-2 hover:font-bold hover:text-[#390278]"
                                  >
                                    {item.name}
                                  </li>
                                ))}
      ...
      <div className="relative border-[1px] rounded-lg mt-5 px-6 py-2 text-[14px] block text-center bg-[#eae8ff]">
